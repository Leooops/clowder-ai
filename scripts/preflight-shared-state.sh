...
if [[ -z "$UPSTREAM" ]]; then
  ...
  if ! git rev-parse "$REMOTE_REF" >/dev/null 2>&1; then
    ...
    UNPUSHED_SHARED=$(git diff --name-only "$MERGE_BASE"..HEAD 2>/dev/null | grep -E '^(docs/ROADMAP\.md|cat-config\.json)$' || true)
  else
    UNPUSHED_SHARED=$(git diff --name-only "$REMOTE_REF"..HEAD 2>/dev/null | grep -E '^(docs/ROADMAP\.md|cat-config\.json)$' || true)
  fi
else
  UNPUSHED_SHARED=$(git diff --name-only "$UPSTREAM"..HEAD 2>/dev/null | grep -E '^(docs/ROADMAP\.md|cat-config\.json)$' || true)
fi

# Also check uncommitted changes to shared state
UNCOMMITTED_SHARED=$(git diff --name-only 2>/dev/null | grep -E '^(docs/ROADMAP\.md|cat-config\.json)$' || true)
STAGED_SHARED=$(git diff --cached --name-only 2>/dev/null | grep -E '^(docs/ROADMAP\.md|cat-config\.json)$' || true)
