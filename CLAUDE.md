...
5. **Worktree Before Code** — Before ANY code edit on a non-main branch, verify you are in a worktree (`.git` is a file, not a directory). If not: STOP, run `/worktree`, then proceed. Enforced by `.githooks/pre-commit`.
